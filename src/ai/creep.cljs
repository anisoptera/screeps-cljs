(ns ai.creep
  ...
  (:use [ai.tower :only [desired-wall-strength]]
        [ai.room :only [load-source init-sources]]))
...
(defn find-container
  [creep & ffn]
  (find-nearest-filter creep js/FIND_STRUCTURES js/STRUCTURE_CONTAINER ffn))
...
(defn find-driveby-repair-target
  [creep]
  (first (room/find-in-range creep js/FIND_STRUCTURES 3
                             #(< 100 (- (structure/max-hits %) (structure/hits %))))))


;; this doesn't work when we pull it out of memory, figure out why.
;; for now, the naive approach isn't _that_ expensive
#_(defn find-driveby-repair-target
  [creep]
  (let [m (creep/memory creep)]
    (if-let [target-id (m "driveby-target")]
      (when-let [target (game/object target-id)]
        (when (pos/in-range? creep (pos/position target) 3)
          target))
      (if (= 0 (mod (game/time) 5))
    ...
    (if (m "dump")
      ...
      (if (= (creep/cargo-amount creep) (creep/energy-capacity creep))
        ...
        (let [ground-score (memoize find-ground-score)
              fills-my-cargo? #(< (creep/energy-capacity creep) (structure/store-quantity %))
              container (memoize #(find-container creep fills-my-cargo?))
              storage (memoize #(find-storage creep fills-my-cargo?))]
          (cond
            ...
            (do
              (perform-with creep (storage creep) structure/transfer-energy)
              (if (pos/next-to? creep (storage creep))
                (creep/memory! creep (assoc m "from-storage" true))))
...
(defn run-rescue
  [creep]
  (let [m (creep/memory creep)]
    (creep/memory! m (assoc m "arch" "courier"))))
