var ScreepsStats = function () {
    screeps.memory.load_memory()
    this.memory = screeps.memory.fetch("___screeps_stats")
    if(!this.memory) {
        screeps.memory.store("___screeps_stats", {})
        screeps.memory.write_memory_BANG_()
        this.memory = {}
    }
    this.username = _.get(
...
ScreepsStats.prototype.memory = null;

ScreepsStats.prototype.clean = function (write) {
    var recorded = Object.keys(this.memory)
    if(recorded.length > 20) {
        recorded.sort()
        var limit = recorded.length - 20
        for(var i = 0; i < limit; i++) {
            this.removeTick(recorded[i], false)
        }

        if (write) {
            screeps.memory.store("__screeps_stats", this.memory);
            screeps.memory.write_memory_BANG_();
        }
    }
}
...
ScreepsStats.prototype.addStat = function (key, value) {
    ...
    if(key_split.length == 1) {
        this.memory[Game.time][key_split[0]] = value
        return
    }

    var start = this.memory[Game.time][key_split[0]]
    ...
    _.merge(start = this.memory[Game.time], tmp)
}
...
ScreepsStats.prototype.runBuiltinStats = function () {
    ...
    this.memory[Game.time] = stats
    screeps.memory.store("___screeps_stats", this.memory)
}
...
ScreepsStats.prototype.removeTick = function (tick) {
    this.removeTick(tick, true)
}

ScreepsStats.prototype.removeTick = function (tick, write) {
    this.memory = screeps.memory.fetch("___screeps_stats")
    
    if(Array.isArray(tick)) {
        for(var index in tick) {
            this.removeTick(tick[index], false)
        }
